...
<head>
  ...
  <title>(github)SUSURU TV.map | 近くのラーメン店</title>
...
　-->
  ...
  function setMarkers(map) {
    ...
    //マーカークラスタリング
    //js_testの配列からbase_locationというlat, lngの配列を作り、マーカークラスタリングができる形にする。

    /*
    const base_location = [];
    for (let i = 0; i < js_test.length; i++) {
      var beach = js_test[i];
      const locations = [lat: parseFloat(beach['latitude']), lng: parseFloat(beach['longitude'])];
      base_location.push(locations);
    ...
    //console.log(base_location);
    /*
    const marker = base_location.map((location, i) => {
...
<script async defer src="https://maps.googleapis.com/maps/api/js?language=ja&region=JP&key=AIzaSyDodZcfXqmf6oAGQs4wTLzCWz2mTwZY-qE&callback=initMap"></script>
